...
- 受けループ選出: メガフシギバナ/メガバンギラス+ドヒドイデ+エアームド
  - 刺さっている相手ならに安定して勝てます。出せるならなるべくこの選出にするのが安定です。メガ枠無しの場合もあります(キュウコン/ドラパルト選出)。
- 自由選出: 自由に3匹
...
- 対ソウブレイズ: メガバギラス必須、優先的にドラパルト/キュウコン
  - 初手出しをメガバンギラスにし、まず初手対面での勝ち筋を狙います。相手ソウブレイズが控えだとして、天候すなあらし状態がきあいのたすきを剥がしてくれるので、そうなればドラパルトのゴースドダイブで確定1発を狙えるようになります。対面でソウブレイズにしっかりダメージを与えられるポケモンを選出して崩しにかかります。
- 対メガピクシー: キュウコン + 保険としてドヒドイデ/(メガ)フシギバナから1匹or(メガ)バンギラス
  - 警戒すべきアシストパワー型メガピクシー相手には、控えにキュウコンを選出しておいて、相手のピクシーが出たらすかさずキュウコンへ交換します。こちらはわるだくみを、相手はコスモパワーを積むレースになりますが、積みレベルが勝った時点で攻撃に転じれば倒せます。対面ならば1回、後出しならば2回わるだくみを積めば良いです。このパーティではキュウコンでしか突破できないので大仕事です。相手がでんじはを使ってきたらバトンタッチ型の可能性が高く、キュウコンの火力に加えて、控えポケモンのどくどくでトドメを刺せるようにしておきたいです。バトンタッチ先がエスパータイプでアシストパワーを使うポケモンの場合、(メガ)バンギラスの選出も視野に入ります。
...
- 技と採用意図
  ...
  - いわなだれ: タイプ一致技です。ストーンエッジの方が威力は高いですが命中率に不安があり、かつ役割対象にはほぼいわなだれで事足りるのでこちらの採用としました。
  - れいとうパンチ: メガボーマンダ対策として採用しました。始めは、いかくでAを下げられても問題なく倒せるようにれいとうパンチではなくれいとうビームの採用を考えましたが、ボーマンダに対してほぼ受け出しするのでいかくを受けることはありませんでした。よってれいとうビームではなくれいとうパンチにしました。
...
- コノヨザル: 選出されやすく、また多くの場合2匹がかりで倒す羽目になります。ちょうはつしてくるのでドヒドイデが無力で、ゴーストタイプなのでエアームドも無力です。メガバンギラスも対面で負けます(相手が先攻でA特化ドレインパンチが乱数1発(6.3%)に対し、こちらのはたきおとす(2回目は威力減少)+砂ダメがH32,B2振り相手に対し確定2発です)。メガフシギバナからはギガドレインで確定4発で、先攻の相手の威力100想定ふんどのこぶしが乱数3発(99.7%)でギガドレインの回復を考慮すると乱数4発(おそらく中~低乱数)で、対面で勝てる可能性があります。威力150想定ふんどのこぶしが乱数2発(98.4%)で、ギガドレインの回復考慮で確定3発、対面負けです。ドラパルトも打ち負けます(ゴーストダイブが乱数1発(6.3%)に対し相手ふんどのこぶし(威力100想定)が確定1発でこちらのりゅうせいぐんだと45.6％～53.9％のダメージです)。キュウコンのにほんばれ下かえんほうしゃは確定2発ですが相手の威力100想定ふんどのこぶしで確定2発、威力150想定で確定1発です。相手コノヨザルがラスト1匹として出たきた場合は対面で勝つ方法がありません。2匹目で出てきた場合はキュウコンが対面で勝てますが、相手がドレインパンチ→ふんどのこぶしとしてきた場合は、回復されてこちらのかえんほうしゃが乱数2発になってしまいます。結論として、コノヨザルを倒すためだけにキュウコン+ドラパルト+メガフシギバナから2匹以上の選出が確定し、突破に2匹要するのでかなり不利です。相手コノヨザルががラスト1匹でない場合はフシギバナで相手をし、ラスト1匹の場合は素早さで勝るキュウコン/ドラパルトで相手をするのが基本になります。
- ソウブレイズ: 選出されやすく、また多くの場合2匹がかりで倒す羽目になります。メガフシギバナ/ドヒドイデ/エアームドは一方的にやられます。キュウコン/ドラパルトは1度だけ攻撃のチャンスがあります。メガバンギラスは唯一単独撃破が可能ですが、相手の型と乱数次第では何も出来ずに倒される可能性があります。基本的には、メガバンギラスが対面で勝つ可能性にかけて立ち回ることになります。もしくは相手がきあいがたすき持ちでないならば、ドラパルトのゴーストダイブで一方的に倒せますが、ゴーストダイブが決まる状況を作り出すのが難しいです。
